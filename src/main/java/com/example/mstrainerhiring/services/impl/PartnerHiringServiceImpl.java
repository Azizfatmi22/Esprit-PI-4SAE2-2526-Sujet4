...
public class PartnerHiringServiceImpl implements PartnerHiringService {
    ...
    public PartnerHiringDTO createPartner(PartnerHiringDTO partnerDTO,
        ...
        partner.setStatus(PartnerStatus.PENDING); // Explicitly set status to PENDING
        PartnerHiring savedPartner = partnerRepository.save(partner);
        log.info("Partner saved with ID: {}", savedPartner.getId());
    ...
    public void deletePartner(UUID id) {
        ...
        // Delete all associated Jobs (which will cascade to Trainer applications)
        ...
        try {
            ...
            if (statusCode < 200 || statusCode >= 400) {
                throw new InvalidFileException(String.format(
                        "Controle de saisie failed: Website '%s' is not reachable (HTTP %d). Please provide a valid, active URL.",
                        website, statusCode));
            }
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            throw new InvalidFileException("Reachability check interrupted for " + website);
        } catch (Exception e) {
            log.warn("Failed to reach website {}: {}", website, e.getMessage());
            throw new InvalidFileException(String.format(
                    "Controle de saisie failed: Could not verify website '%s'. Please make sure it exists and is publicly accessible.",
                    website));
        }
    }
    ...
    private void validateWebsiteDomain(String email, String website) {
        ...
        if (!website.toLowerCase().contains(domain)) {
            throw new InvalidFileException(String.format(
                    "Controle de saisie failed: Website '%s' must contain the domain name '%s' from the business email.",
                    website, domain));
        }
    }
